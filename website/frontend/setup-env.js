...
const slabAccountPath = path.join(__dirname, '../../private/slab-account.json');
const routerInfoPath = path.join(__dirname, '../../private/router-info.json');

if (!fs.existsSync(slabAccountPath)) {
  console.error('❌ Error: private/slab-account.json not found');
  console.log('   Run scripts/initialize-slab.ts first to deploy the slab program\n');
  process.exit(1);
}

if (!fs.existsSync(routerInfoPath)) {
  console.error('❌ Error: private/router-info.json not found');
...
console.log('✅ Found deployed program info:\n');
...
console.log('✅ Created .env.local with deployed addresses\n');
