...
export function RecentTrades({ 
  ...
  return (
    <div className={cn(
      ...
      {/* Header */}
      <div className="h-10 sm:h-12 flex items-center justify-between px-3 sm:px-4 border-b border-[#181825]">
      ...
      <div className="p-2 sm:p-3">
        {loading ? (
          ...
          <>
            {/* Column Headers */}
            <div className="grid grid-cols-4 gap-1 sm:gap-2 text-[9px] sm:text-xs text-gray-400 mb-2 font-semibold">
              <span>Side</span>
              <span>Price</span>
              <span>Size</span>
            ...
            {/* Trades */}
            <div className="space-y-0.5 sm:space-y-1 max-h-[300px] sm:max-h-[400px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {displayTrades.map((trade, index) => (
                <div 
                  key={index} 
                  className="grid grid-cols-4 gap-1 sm:gap-2 text-[10px] sm:text-xs sm:text-sm py-1 sm:py-1.5 hover:bg-[#181825]/50 rounded px-1 group cursor-pointer transition-colors"
                  onClick={() => {
                ...
                >
                  <div className="flex items-center gap-1 sm:gap-1.5">
                    {trade.side === 'buy' ? (
                      <>
                        ...
                        <span className="text-green-400 font-semibold text-[9px] sm:text-xs">BUY</span>
                      </>
                    ) : (
                      <>
                        <TrendingDown className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-400" />
                        <span className="text-red-400 font-semibold text-[9px] sm:text-xs">SELL</span>
                      </>
                    )}
                  </div>
                  ...
                  <div className="text-right flex items-center justify-end gap-1">
                    <span className="text-gray-400 text-[9px] sm:text-xs" suppressHydrationWarning>
                    ...
                    {trade.solscanLink && (
                      <ExternalLink className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                    )}
                  </div>
                </div>
              ))}
